...
func livekitWebhook(streamRepo *sqlite.StreamRepo, roomRepo *sqlite.RoomRepo, provider *auth.SimpleKeyProvider) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Only handle stream-participant identities ("stream:<room_id>")
		...
		if err := roomRepo.SetActiveStream(ctx, stream.RoomID, nil); err != nil {
			slog.Error("webhook: failed to clear active stream", "room_id", stream.RoomID, "error", err)
		}

		slog.Info("webhook: cleaned up dead stream", "room_id", stream.RoomID, "ingress_id", ingress.IngressId, "stream_session_id", stream.StreamSessionID)
		w.WriteHeader(http.StatusOK)
	}
}
