...
func (s *RoomService) MuteParticipant(ctx context.Context, roomID, requesterSessionID, targetSessionID string, mute bool) error {
	...
	// Short-circuit idempotent requests: already in the requested state.
	// Avoids an unnecessary LiveKit call and rollback ambiguity.
	if target.MutedByOwner == mute {
		return nil
	}
	...
	if err := s.lkRoomService.MuteParticipantTrack(ctx, roomID, targetSessionID, mute); err != nil {
		// Rollback DB to the authoritative prior value to maintain consistency
		if rbErr := s.participants.SetMutedByOwner(ctx, targetSessionID, target.MutedByOwner); rbErr != nil {
